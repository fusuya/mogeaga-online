...
;;ダンジョンのデータリスト (block enemies item dmg)
...
;;(type (players) events)
;; players (((player-data) (donjon-data)) ...)
...
(defun game-broadcast-result (g)
  (game-broadcast-message
   g
   `(2 ,(make-ranking-data g))))

(defun game-broadcast-status (g timeout-seconds)
  (game-broadcast-message g `(:|type| 0 :|timeout-seconds| ,timeout-seconds
			       :|map| ,(make-donjon-data g))))


(defun game-broadcast-quit (rp)
  (handler-case
      (remote-player-send-message rp `(4))
    (sb-int:simple-stream-error
	(c)
      (declare (ignore c))
